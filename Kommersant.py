...
class KommersantNewsParser:

    def __init__(self, base_url: str = 'https://www.kommersant.ru/'):
        """
        ...
        self.url_helper = 'https://www.kommersant.ru'
        ...
        self.serious = ["/rubric/3?from=burger", "/rubric/2?from=burger", "/tech?from=burger", '/rubric/8?from=burger', "/rubric/9?from=burger"
                        ]
    ...
    def get_main_page_links(self) -> List[str]:
        ...
        try:
            ...
            allfd = soup.find_all('li', class_="site_menu__item")

            links = []
            for href in allfd:
                link_element = href.find('a', href=True)
                if link_element:
                    if (link_element['href'] == "/rubric/2?from=burger"):
                        self.categ[link_element['href']] = "Политика"
                        links.append(link_element['href'])
                    elif (link_element['href'] == "/rubric/8?from=burger"):
                        self.categ[link_element['href']] = "Культура"
                        links.append(link_element['href'])
                    elif (link_element['href'] == "/rubric/9?from=burger"):
                        self.categ[link_element['href']] = "Спорт"
                        links.append(link_element['href'])
                    elif (link_element['href'] == '/rubric/3?from=burger'):
                        self.categ[link_element['href']] = "Экономика"
                        links.append(link_element['href'])
                    elif (link_element['href'] == '/tech?from=burger'):
                        self.categ[link_element['href']] = "Наука"
                        links.append(link_element['href'])

            print(links, self.categ)
            return links
        except Exception as e:
    ...
    def _parse_article_page(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            title_elem = soup.find_all('h1', class_="doc_header__name js-search-mark")
            for element in title_elem:
                title += element.get_text()
            # Извлечение даты публикации
            date_elem = soup.find_all('time', class_="doc_header__publish_time")
            for element in date_elem:
                publish_date += element.get_text()
            # Извлечение содержимого статьи
            content_elem = soup.find_all('p', class_="doc__text")
        ...
        for link in category_links:
            ...
            try:
                ...
                if link in self.serious:
                    news_blocks = soup.find_all('h2', class_='uho__name rubric_lenta__item_name')
                else:
                    continue
                ...
                for news_block in news_blocks:
                    ...
                    for a_tag in a_tags:
                        href = a_tag['href']
                        href = self.url_helper + href
                        if href and href.startswith('http') and href not in hrefs_seen and i < 5:
                            news_item = NewsItem(
                ...
                print(f"Категория {self.categ[link]}: найдено {len(hrefs_seen)} статей из этой страницы")
    ...
    def save_to_json(self, news_items: List[NewsItem], filename: str = 'Kommersant_news.json'):
        """Сохранение результатов в JSON файл"""
        data = []
        for item in news_items:
            if item.content:  # Сохраняем только статьи с контентом
                data.append({
    ...
    def run_full_parsing(self, output_file: str = 'Kommersant_news.json') -> List[NewsItem]:
        """
        ...
        """
        print("=== Начало парсинга Lenta.ru ===")
